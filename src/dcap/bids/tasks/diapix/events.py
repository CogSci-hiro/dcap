...
def _match_intervals_delay_mad(
    *,
    ...
    for raw_idx, raw_int in enumerate(raw_intervals):
        wav_idx = np.abs(wav_intervals - raw_int).argmin()
        if abs(raw_int - wav_intervals[wav_idx]) > tolerance:
            continue
        onset_diffs.append(raw_onsets[raw_idx] - wav_onsets[wav_idx])

    if len(onset_diffs) == 0:
        raise ValueError("Could not match any intervals; check trigger extraction or tolerance.")

    diffs = np.asarray(onset_diffs, dtype=float)
